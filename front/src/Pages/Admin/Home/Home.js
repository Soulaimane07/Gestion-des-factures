    ...
    <>
        ...
        <div className='min-h-screen px-20 py-16 bg-gray-50 text-gray-700'>
          <div className="grid gap-6 mb-8  md:mb-12 md:grid-cols-2">
              {cards?.map((item,key)=>(
              <Link to={item.link} key={key} className="flex flex-col shadow-md rounded-md p-8 bg-white text-gray-700 border-2 border-opacity-5 border-gray-600">
                  <h2 className='text-xl font-semibold '> {item.title} ({item.number ?? 0}) </h2> 
                  <Link to={item.link} className=' mt-4 hover:text-orange-500 transition-all'> Read more... </Link>  
              </Link>
              ))}
          </div>
        </div>
